...
totalresult=[];
data = pd.read_csv("primary.csv",low_memory=False);
result = [];
...
with open("tertiary.csv", 'w') as csvfile:
            fieldnames = ['TotalValidblocks', 'Leastblocksize','MaxBlocksize','Leastblockgaslimit','Maxblockgaslimit','Leastblockgasused','Maxblockgasused','Leastblocktransactions','Maxblocktransactions','blocktimeperiod','totaltxs','finaltps','firedtps','totalfiredtime','totalblocktimeinterval','txgasused','TotalUncleblocks']
            writer = csv.DictWriter(csvfile, fieldnames=fieldnames)

            writer.writeheader();
            writer.writerow({'TotalValidblocks':result[0], 'Leastblocksize':result[1],'MaxBlocksize':result[2],'Leastblockgaslimit':result[3],'Maxblockgaslimit':result[4],'Leastblockgasused':result[5],'Maxblockgasused':result[6],'Leastblocktransactions':result[7],'Maxblocktransactions':result[8],'blocktimeperiod':result[9],'totaltxs':result[10],'finaltps':result[11],'firedtps':result[12],'totalfiredtime':result[13],'totalblocktimeinterval':result[14],'txgasused':txgasused,'TotalUncleblocks':Totaluncleblocks})
