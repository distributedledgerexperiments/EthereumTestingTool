...
echo Nissan!123 | sudo -S systemctl stop docker

echo Nissan!123 | sudo -S cp daemon.json ~/../../etc/docker/

echo Nissan!123 | sudo -S systemctl start docker

#Twice to change later
echo Nissan!123 | sudo chmod 666 /var/run/docker.sock

docker run -d -p 5000:5000 --restart=always --name registry registry

docker-compose -f dockercomposebase.yml build

docker-compose -f dockercomposebase.yml push

docker-compose -f dockercompose15.yml up -d
