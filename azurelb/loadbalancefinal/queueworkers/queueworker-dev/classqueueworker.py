...
class QueueWorker(threading.Thread):
    ...
    def threadfunction(self,redisnonce,channel,method, properties, body):
     ...
     if 'result' not in jsonresponse:
        resulterror={'Transactionid':jsonresponse['error'],'Start':starttime,'End':endtime,'Node':self.nodetype,'Count':redisnonce,'host':str(self.host),'Worker':str(self.workertype)}
        try:
        ...
        resultdocument={'Transactionid':str(jsonresponse['result']),'Start':starttime,'End':endtime,'Node':self.nodetype,'Count':redisnonce,'host':str(self.host),'Worker':str(self.workertype)}
        try:
    ...
    def __init__(self):
        ...
        #Dynamic Params
        self.geth_ws = "ws://10.214.226.143:2200"
        self.host="10.214.226.146"
        self.workertype="A"
        self.nodetype="Node2"

        self.maincontractaddress="0x47049a977274eaeddcf11bf7af62615421d55500"
        self.mongodbhost='10.214.226.37'
        self.mongodbport='28017'
        self.redishost='172.25.0.101'
        self.redisinstance=[(self.redishost,6379)]
        self.rabbithost='172.25.0.102'
