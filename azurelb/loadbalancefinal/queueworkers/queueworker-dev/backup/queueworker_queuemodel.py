...
mongodbhost='10.214.226.37'
mongodbport='27017'
redishost='172.25.0.101'
...
def printResult():
   ...
   for row in totalresult:
       resultdocument={'Transactionid':row[0],'Start':row[1],'End':row[2],'Count':row[3],'host':str(host)+str(workertype)}
       resultdocuments.append(resultdocument)
   try:
       client=MongoClient(host=[str(mongodbhost)+':'+str(mongodbport)],serverSelectionTimeoutMS=3000,username='guest',password='guest')
       db = client.resultdb
       results = db.results
       idf=results.insert_many(resultdocuments)
       totalresult = []
