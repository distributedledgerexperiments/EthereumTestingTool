...
def deployContract(contract_interface, ifPrint=True):
    ...
    walletnonce = w3.eth.getTransactionCount(w3.eth.defaultAccount)
    contracttx =  myContract.constructor().buildTransaction({
        'gas': 900000, 'from': w3.eth.defaultAccount, 'nonce': walletnonce
        })
    signed = w3.eth.account.signTransaction(contracttx, NODE_PRIVATE_KEY)
...
def testMethods(myContract):
    ...
    answer = myContract.functions.readAdd().call()
    ...
    tx_hash = w3.toHex( myContract.functions.addTest(100).transact(txParameters) )
