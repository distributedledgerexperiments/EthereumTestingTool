...
def contract_set_via_RPC(contract, arg, privateFor=PRIVATE_FOR, gas=900000):
    """
    ...
    method_ID = contract_method_ID("addTest", contract.abi)
    data = argument_encoding(method_ID,100)
    txParameters = {'from': w3.eth.defaultAccount, 
                    'to' : contract.address,
                    'gas' : w3.toHex(gas),
                    'gasPrice': w3.toHex(1),
    ...
    if privateFor:
        txParameters['privateFor'] = privateFor  # untested
    signedtx=w3.eth.account.signTransaction(txParameters,privatekey)    
    method = 'eth_sendRawTransaction'
    payload= {"jsonrpc" : "2.0",
               "method" : method,
               "params" : [w3.toHex(signedtx.rawTransaction)],
               "id"     : 1}
...
def test_contract_set_via_RPC(contract, steps=3):
    ...
    for number in range(rand, rand+steps):
        ...
        #storedData = contract.functions.get().call()
        storedData = contract.functions.readAdd().call()
        print (storedData) 
...
if __name__ == '__main__':
    ...
    w3.eth.defaultAccount =  w3.toChecksumAddress(NODE_ACCOUNT)
    #w3.eth.accounts[0] # set first account as sender
