...
def start_web3connection(RPCaddress=None, account=NODE_ACCOUNT):
    """
    ...
    accountname="chosen"
    if not account:
        w3.eth.defaultAccount = Web3.toChecksumAddress(NODE_ACCOUNT)
        #w3.eth.accounts[NODE_ACCOUNT] # set first account as sender
        accountname="first"
...
def unlockAccount(duration=3600, account=None):
    ...
    if PARITY_UNLOCK_EACH_TRANSACTION:
        ...
        #answer = w3.personal.unlockAccount(account=account,passphrase=passphrase,duration=duration)
    return True
...
def web3connection(RPCaddress=RPCaddress, account=NODE_ACCOUNT):
    """
...
if __name__ == '__main__':
    ...
    answer = web3connection(RPCaddress=RPCaddress, account=NODE_ACCOUNT)
