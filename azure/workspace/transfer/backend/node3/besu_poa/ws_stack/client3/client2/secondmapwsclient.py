...
def sendTransaction():
   ...
   def worker():
        while True:
            item = q.get()
            threadfunction(item,contract,totalnonce,gashex,gaspricehex,totalresult,ws)
            q.task_done()
   #num_worker_threads
...
def threadfunction(increment,contract,totalnonce,gashex,gaspricehex,totalresult,ws):
  result=[]
  starttime=int(round(time.time()))

  txparam=contract.functions.secondMapTest(999999999,88888888).buildTransaction({
    'chainId':int(26),
    'from': account,
